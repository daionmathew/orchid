...
for r in rooms:
    ...
    # Stock Issues
    print("\nStock Issues:")
    issues = db.execute(text("SELECT i.name, sid.issued_quantity, sid.is_payable, si.issue_date, si.booking_id, si.notes FROM stock_issue_details sid JOIN stock_issues si ON sid.issue_id = si.id JOIN inventory_items i ON sid.item_id = i.id WHERE si.destination_location_id = :loc_id"), {"loc_id": loc_id}).fetchall()
    for iss in issues:
