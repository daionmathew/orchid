...
def read_all(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), skip: int = 0, limit: int = 20):
    return _read_all_impl(db, skip, limit)

@router.get("/", response_model=list[FoodCategoryOut])  # Handle trailing slash
def read_all_slash(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), skip: int = 0, limit: int = 20):
    return _read_all_impl(db, skip, limit)
